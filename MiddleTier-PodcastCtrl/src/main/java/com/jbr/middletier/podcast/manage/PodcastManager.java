...
public class PodcastManager {
    ...
    @Value("${middle.tier.podcast.deletedirectory}")
    private String oldPodsDirectory;

    @Value("${middle.tier.podcast.daystokeep:30}")
    private int daysToKeep;

    @Autowired
    public PodcastManager(PodcastRepository podcastRepository,
    ...
    private void downloadData(List<Podcast> podcastList) {
        for (Podcast podcast : podcastList) {
            try {
                RSSPodcastFeedParser parser = new RSSPodcastFeedParser(logType, podcastEpisodeRepository, podcast);
    ...
    private void deleteFile(Podcast podcast, PodcastEpisode episode) {
        try {
            ...
            if(podcastFile.exists()) {
                ...
                File directoryForOldPods = new File(oldPodsDirectory);
                if(!directoryForOldPods.exists()) {
                    throw new Exception(String.format("%s does not exist", oldPodsDirectory));
                }
                Path directory = Paths.get(String.format("%s/%s",oldPodsDirectory,episode.getFilename()));
                ...
                touchFile(String.format("%s/%s",oldPodsDirectory,episode.getFilename()));
            }
    ...
    private void checkForFilesToRemove(List<Podcast> podcastList) {
        try {
            ...
            File directory = new File(oldPodsDirectory);
            if(!directory.exists()) {
                if(!directory.mkdir()) {
                    throw new IllegalStateException(String.format("Old Pods Directory %s does not exist.", oldPodsDirectory));
                }
            }
    ...
    private void removeOldFiles() {
        try {
            ...
            File folder = new File(oldPodsDirectory);
            File[] listOfFiles = folder.listFiles();
            ...
            if(listOfFiles != null ) {
                for (File oldPodFile : listOfFiles) {
                    ...
                    if (diff > daysToKeep * 24 * 60 * 60 * 1000) {
